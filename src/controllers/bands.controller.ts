...
export const joinBand = async (req: AuthRequest, res: Response) => {
  ...
  try {
    const bandResult = await pool.query(
      "SELECT * FROM bands WHERE invite_code = $1",
...
export const deleteBand = async (req: AuthRequest, res: Response) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM bands WHERE id = $1 AND owner_id = $2 RETURNING *",
      [id, userId],
    );
...
export const updateMemberPermissions = async (req: AuthRequest, res: Response) => {
  ...
  if (typeof can_create_gigs !== "boolean") {
    return res.status(400).json({ error: "can_create_gigs debe ser un booleano" });
  }
  ...
  try {
    ...
    if (bandCheck.rowCount === 0) {
      return res.status(403).json({ error: "Solo el encargado puede modificar permisos" });
    }
    ...
    if (result.rowCount === 0) {
      return res.status(404).json({ error: "Miembro no encontrado en esta banda" });
    }
  ...
  try {
    ...
    if (bandResult.rows[0].owner_id === userId) {
      return res.status(400).json({
        error:
          "El encargado no puede salir de la banda. Elimínala en su lugar.",
      });
    }
