...
export async function checkGigReminders(): Promise<void> {
  ...
  try {
      ...
      `
        ...
        AND NOT EXISTS (
          ...
          WHERE gnd.gig_id = r.gig_id
            ...
            AND gnd.notification_type = 'gig_reminder_24h'
    ...
    for (const [gigId, rows] of remindersByGig.entries()) {
      ...
      const formattedTime = String(first.gig_time).slice(0, 5);

      const placeText = first.place
        ? ` en ${first.place}`
        : "";
      ...
      try {
        await sendPushToUsers({
          userIds,
          title: "Tocada mañana",
          body: `"${first.title}" es mañana a las ${formattedTime}${placeText}.`,
          data: {
            type: "gig_reminder_24h",
            gigId,
        ...
        for (const row of rows) {
          await pool.query(
            `
              ...
              VALUES ($1, $2, 'gig_reminder_24h', $3, $4)
        ...
        console.log(
          `Recordatorio 24h enviado para tocada ${gigId} a ${userIds.length} usuario(s)`,
        );
      } catch (error) {
        console.error(
          `Error enviando recordatorio 24h de tocada ${gigId}:`,
          error,
        );
      }
    }
